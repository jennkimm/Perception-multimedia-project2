...
vec4 seeThrough(float yc, vec2 p, mat3 rotation, mat3 rrotation)
{
    ...
    vec4 color = texture(tex0, point.xy);
...
void main() {
    ...
    bgColor = texture(tex0, uv).rgba;
    
    curlExtent = sin(u_time*0.3)*0.5+0.5;
    
    if (u_mouse.z>0.) curlExtent = u_mouse.y/u_resolution.y;
